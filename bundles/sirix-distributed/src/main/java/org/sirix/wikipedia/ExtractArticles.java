 ...
 * found in the XML document. The threshold value N can be set by the user or
 * defaults to 10.
 * </p>
 * 
...
public final class ExtractArticles extends XMLFilterImpl {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ExtractArticles.class);
	...
	private static final QName WIKIROOT = new QName(XMLConstants.NULL_NS_URI,
			"mediawiki", XMLConstants.DEFAULT_NS_PREFIX);
	...
	public void startElement(final String paramURI, final String paramLocalName,
		...
		if (mArtParsed <= mArtNr) {
			...
			// After mArtNr articles have been parsed close root element / end
			// document and exit.
			super.endElement(WIKIROOT.getNamespaceURI(), WIKIROOT.getLocalPart(),
					WIKIROOT.getPrefix() + ":" + WIKIROOT.getLocalPart());
			super.endDocument();
